...
class AppointmentService {
     ...
     */
    async listAppointments(psychologistId, filters) {
     ...
     */
    async getAppointmentsByDate(psychologistId, date) {
     ...
     */
    async getAppointment(appointmentId, psychologistId) {
     ...
     */
    async createAppointment(psychologistId, data) {
        ...
        if (!patient) {
            throw new errorHandler_1.AppError('Paciente não encontrado ou inativo', 404);
        }
        ...
        if (data.serviceId) {
            ...
            if (!service) {
                throw new errorHandler_1.AppError('Serviço não encontrado ou inativo', 404);
            }
        }
        ...
        if (conflictingAppointment) {
            throw new errorHandler_1.AppError('Já existe um agendamento neste horário', 400);
        }
     ...
     */
    async updateAppointment(appointmentId, psychologistId, data) {
        const appointment = await prisma_1.prisma.appointment.findFirst({
            where: {
                id: appointmentId,
                psychologistId,
        ...
        if (appointment.status === 'CANCELLED') {
            throw new errorHandler_1.AppError('Não é possível atualizar um agendamento cancelado', 400);
        }
        ...
        if (data.status === 'COMPLETED' &&
            ...
            if (sessionNotes.trim() && this.aiService.isConfigured()) {
                try {
                    ...
                    const newSummary = await this.aiService.generateSessionSummary(sessionNotes, appointment.patient.name, appointment.patient.clinicalSummary);
     ...
     */
    async cancelAppointment(appointmentId, psychologistId) {
        const appointment = await prisma_1.prisma.appointment.findFirst({
            where: {
                id: appointmentId,
                psychologistId,
        ...
        if (appointment.status === 'COMPLETED') {
            throw new errorHandler_1.AppError('Não é possível cancelar um agendamento já concluído', 400);
        }
        if (appointment.status === 'CANCELLED') {
            throw new errorHandler_1.AppError('Agendamento já está cancelado', 400);
        }
     ...
     */
    async getAppointmentStats(psychologistId) {
     ...
     */
    async getActiveSession(psychologistId) {
        const activeSession = await prisma_1.prisma.appointment.findFirst({
            where: {
                psychologistId,
                status: 'IN_PROGRESS',
            },
